...
> **すでに稼働中のインスタンスを更新する場合:** デプロイ後に **`setupTriggers()` を1度手動実行**してください。「師匠リスト」シートの作成・編集トリガーの登録に加え、`MASTER_SLACK_USER_IDS` に手で入れていた師匠がシートへ自動で移し替えられます（移行された行はメモ欄に「自動移行」と記録されます）。実行しないと、シートを編集しても反映されません。
