...
2. GASエディタ上部の関数選択プルダウンで **`bootstrap`** を選び、「**実行**」を押します（初回は権限承認ダイアログが出るので許可）。以下が自動で行われます。
   ...
   - 弟子用・師匠用フォームを規定の設問構成（後述の「フォームの設問構成」）で作成
   - イベント用カレンダーを作成
...
2. GASエディタで **`setupTriggers`** を選んで**手動で1回実行**します。以下が自動で行われます。
   ...
   - 両スプレッドシートへのシート自動作成
   - コード側の定義にあってフォームに無い設問の追加（既存の設問は変更しません）
   - 「開催形式」の選択肢と、「VC部屋」の選択肢（「VCルームリスト」シートの内容）の反映
   - フォームの「開催形式」「VC部屋」「会場URL または 開催場所」の設問への入力ヒント設定（無料版Meetの60分制限と予定分割の説明など）
...
`bootstrap()` が生成するフォームは以下の設問構成（コード上の `formSpec_()`）です。**設問タイトルは下表と完全一致**している必要があります（GASはタイトル文字列で回答を照合するため）。手動でフォームを作り直す場合の参照用に残します。
...
| 7 | 開催形式 | ラジオボタン「Discord VC / Google Meet / その他のURL / 対面（オフライン）」 | ✓ |
| 8 | VC部屋 | ラジオボタン「おまかせ（自動割り当て）」＋「VCルームリスト」シートのVC名（シート編集時に自動同期） | － |
| 9 | 会場URL または 開催場所 | 記述式（Discord VC・Google Meet の場合は空欄可） | － |
| 10 | 概要・対象者 | 段落 | ✓ |
| 11 | 事前準備・持ち物・資料リンク | 段落 | － |

> 開催形式の選択肢の文字列は `Config.gs` の `EVENT_FORMATS` が正で、GASはこの値と**完全一致**で判定します。フォーム側で文言を書き換えると、送信が「選択肢にない開催形式」として差し戻されます。
...
### 「師匠リスト」シートから自動生成されるもの

| プロパティ名 | 自動生成される値 |
|---|---|
| `MASTER_SLACK_USER_IDS` | 師匠のSlackユーザーID（カンマ区切り）。`/event` で師匠用フォームのリンクを返す相手の判定に使う |
| `MASTER_LIST_MIGRATED` | 移行済みフラグ（内部用）。詳細は下記 |

**この2つは手で編集しないでください。** 誰が師匠かは、管理用スプレッドシートの **「師匠リスト」シート** で管理します（運用手順は [OPERATIONS.md](OPERATIONS.md) の「師匠の追加・削除」を参照）。シートを操作した時点でトリガーが走り、`MASTER_SLACK_USER_IDS` が自動で書き換わります。

トリガーは2つ登録されます。**`onEdit`**（セルの値の変更）と **`onChange`**（行の削除・シートの削除）です。`onEdit` はセルの値が変わったときにしか発火せず、行を削除しても動かないため、「行ごと削除して師匠を外す」操作を拾うには `onChange` が必要です。

`/event` の応答経路でシートを直接読まずにプロパティを経由しているのは、Slackの3秒ルール対策です。`/event` の処理時間は実測で約0.6秒（GASの実行数ダッシュボードの値。ネットワーク往復とコンテナ起動を含まない）で、ここに `SpreadsheetApp.openById()` の0.3〜1秒を毎回足すと超過リスクが上がるため、シートの読み取りは3秒ルールの外にある編集トリガー側へ寄せています。

`MASTER_LIST_MIGRATED` は、以前 `MASTER_SLACK_USER_IDS` を手で登録して運用していたインスタンスを1度だけシートへ移行するためのフラグです。移行後は「シートが空＝師匠0名」として素直に扱われます（フラグが無いと、最後の1人を消したときに古いプロパティ値から復活してしまうため）。

> **すでに稼働中のインスタンスを更新する場合:** デプロイ後に **`setupTriggers()` を1度手動実行**してください。コード側で設問を増やした場合も、稼働中のフォームへ追随させる経路はここだけです（`syncFormItems()` が、定義にあってフォームに無い設問だけを追加します。既存の設問は変更・削除しません）。「師匠リスト」シートの作成・編集トリガーの登録に加え、`MASTER_SLACK_USER_IDS` に手で入れていた師匠がシートへ自動で移し替えられます（移行された行はメモ欄に「自動移行」と記録されます）。実行しないと、シートを編集しても反映されません。
