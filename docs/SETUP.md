...
**この2つは手で編集しないでください。** 誰が師匠かは、管理用スプレッドシートの **「師匠リスト」シート** で管理します（運用手順は [OPERATIONS.md](OPERATIONS.md) の「師匠の追加・削除」を参照）。シートを編集した時点で編集トリガーが走り、`MASTER_SLACK_USER_IDS` が自動で書き換わります。
