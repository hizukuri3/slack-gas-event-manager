...
- **設定内容:** 1行に1人を書きます。
  ...
  | 有効 | 空欄か `TRUE` で有効、`FALSE` で一時停止（行を消さずに外せます） |
  | メモ | 処理には使わない自由記入 |

- **反映のタイミング:** シートを編集した**その場で自動反映**されます。反映されると画面右下に「師匠リスト: 有効な師匠 N名を反映しました」とトーストが出るので、これを反映の確認に使ってください。
- **師匠を外すとき:** 行を削除するか、「有効」列に `FALSE` を入れます。どちらでも次に `/event` を打ったときから弟子用フォームのリンクが返るようになります。
...
- **トーストは出たが届かない**: 「有効」列に `FALSE` が入っていないか、`MASTER_FORM_ID`（師匠用フォーム）が未設定でないかを確認してください。師匠用フォームが未設定の場合は、師匠にも弟子用フォームのリンクが返ります。
