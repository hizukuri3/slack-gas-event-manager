...
- **開催前日の夜と開始直前に、リマインドDMが自動で届く**（事前準備の内容・Meet URL・告知へのリンク付き）。カレンダーを見ていなくても参加し忘れない。
...
flowchart LR
    ...
    subgraph Slack
        ...
        DM[DM<br>編集用URL・繰り上げ通知<br>開催前リマインド]
    end
    Form -->|送信トリガー| GAS
    GAS --> Cal
    GAS --> SS1
...
4. メンバーへの購読案内は不要です。告知メッセージのフッターに出る「:calendar: カレンダーを開く」が、Googleカレンダーの「共有可能なリンク」と同じ形式（`?cid=…`）で自動生成されるため、**メンバーは初回に一度押すだけで自分のカレンダーに追加**されます（→ 3. メンバーへ事前に案内すること）。
...
2. GASエディタで `setupTriggers` 関数を選択して**手動で1回実行**します（初回は権限承認ダイアログが表示されるので許可してください）。この実行で以下が自動で行われます。
   - フォーム送信トリガー・公開用シート定期同期トリガー（5分毎）・開催前リマインドトリガー（1時間毎）の登録
   - 両スプレッドシートへのシート自動作成
...
参加登録済みのメンバーへ、**開催前に2回リマインドDM**が自動で届きます（1時間毎の時間トリガーが判定します。設定は不要です）。

| タイミング | 送信条件 | 内容 |
|---|---|---|
| 前日の夜 | 前日20時以降の実行時 | 日時・場所・主催者・**事前準備の内容**・告知へのリンク・キャンセルの案内 |
| 開催直前 | 開始まで残り90分を切ったとき | 日時・場所（Meet URL）・主催者・告知へのリンク |

- 送り先は状態が **「参加」「運営」の人のみ**です。**キャンセル待ちの人には送りません**（参加が確定していないため。繰り上がった時点で別途DMが届きます）。
- 中止されたイベント、および既に開始時刻を過ぎたイベントには送信されません。
- 送信済みは記録されるため、トリガーが複数回動いても**同じリマインドが二重に届くことはありません**。
- 時間トリガーの発火時刻には数分〜十数分のゆらぎがあるため、「ちょうど1時間前」ではなく**開始の30〜90分前**に届きます。
- タイミングを変えたい場合は `src/ReminderService.gs` 冒頭の `REMINDER_PREV_DAY_HOUR`（前日の送信時刻・既定20時）と `REMINDER_BEFORE_MINUTES`（直前リマインドの猶予・既定90分）を編集してください。

**なぜGoogleカレンダーの通知ではなくSlack DMなのか:**
カレンダーAPIの通知設定（`reminders`）は**予定を作成したアカウント本人にしか効きません**。参加者のカレンダーへ通知を出すにはゲスト招待＝**メールアドレスの取得が必要**になり、本システムの「メールアドレスを一切収集しない」方針（2章「個人情報保護」）と両立しないため、リマインドはSlack側で完結させています。
...
└── src/
    ...
    ├── ReminderService.gs   # 開催前リマインドDM（前日夜・開催直前）
