...
参加者リストの書き込みは、ボタン応答を高速に保つため**管理用のみリアルタイム**で行い、**公開用へは5分毎の時間トリガーで自動同期**します（変更がないときは即終了。全行上書きのため途中失敗しても次回同期で必ず一致します）。リアルタイムの参加状況はSlack告知メッセージと参加者確認Webページで確認できます。イベントマスターは従来どおり両方へ即時反映されます。
...
2. GASエディタで `setupTriggers` 関数を選択して**手動で1回実行**する（初回は権限承認ダイアログが表示されるので許可する）。
   - フォーム送信トリガー・公開用シート定期同期トリガー（5分毎）の登録と、両スプレッドシートへのシート自動作成が行われます。
   - あわせて、フォームの「開催形式」「会場URL または 開催場所」の設問に**入力ヒント（無料版Meetの60分制限と予定分割の説明）が自動設定**されます。
3. 続けて `setupPrefillEntryIds` 関数（SlashCommand.gs）も**手動で1回実行**する。
...
| `FORM_ENTRY_ORGANIZER` | 弟子用フォーム「主催者のSlackユーザーID」設問のエントリID。**`setupPrefillEntryIds()` の実行で自動登録**（手動登録の場合は `entry.123456` の数値部分、`entry.` 付きでも可） | 推奨※ |
| `FORM_ENTRY_STATUS` | 弟子用フォーム「イベントのステータス」設問のエントリID。同上 | 推奨※ |
| `FORM_STATUS_OPEN_VALUE` | ステータス設問の「開催」を表す選択肢の文字列（フォームの選択肢と完全一致）。同上 | 推奨※ |
| `MASTER_FORM_ENTRY_ORGANIZER` | 師匠用フォームの「主催者のSlackユーザーID」設問のエントリID。同上 | 任意※ |
| `MASTER_FORM_ENTRY_STATUS` | 師匠用フォームの「イベントのステータス」設問のエントリID。同上 | 任意※ |

※ エントリID系プロパティは未設定でも `/event` は動作しますが、リンクが事前入力なしの素のフォームURLになります。Slackの3秒ルール対策として、`/event` の応答経路では `FormApp` を使わず、これらの固定値との文字列結合だけで事前入力URLを組み立てています。
