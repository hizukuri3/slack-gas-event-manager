...
- **告知の本文はひと目で判断できる重要情報（イベント名・開催日時・場所・主催・参加状況・参加ボタン）だけに絞り、概要・対象者・持ち物などの詳細は同じメッセージの「スレッド」に投稿**します。本文が長くて読みづらいというフィードバックへの対応です。詳細を更新（回答編集）するとスレッド側の投稿も自動で最新化されます。
