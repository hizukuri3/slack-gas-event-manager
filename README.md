...
- **告知の本文には、イベント名・開催日時・場所・主催・概要（対象者）・参加状況・参加ボタンといった判断に必要な情報を載せ、事前準備・持ち物・資料リンクは同じメッセージの「スレッド」に投稿**します。本文が長くて目が泳ぐというフィードバックへの対応です。概要は「内容が分かってスレッドを開く気になる」よう、本文の上部に**省略せず全文**載せます。
- **詳細スレッドは登録時に必ず1件先に投稿して「場所」を確保**します。事前準備・持ち物が未記入のときは「登録されていません（追加されるとここに表示されます）」と表示します。以降の更新（回答編集）は**この1件をその場で書き換える（`chat.update`）**ため、後から持ち物を追加しても**投稿の順番はずれません**（満席・空き枠・日時変更などのお知らせより常に上に留まります）。新しい返信として積み直すことはしません。
- なお本文からスレッドへの導線（「事前準備・持ち物はスレッドをご覧ください」）は、実際に詳細が入力されているときだけ表示します（中身が空のスレッドは無理に開かせません）。
